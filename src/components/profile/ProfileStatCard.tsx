    ...
    if (rank && typeof rank === 'string' && rank.includes('/') && id !== 'topGenres') {
        ...
        rankDisplay = (
            <span className="text-sm font-normal text-slate-400 ml-2 whitespace-nowrap ">
                <HashtagIcon
                    className="inline h-3 w-3 mr-0.5 relative -top-px"
                    aria-hidden="true"
                />
                {rankNum}
        ...
        >
            <div>
                <div className="flex items-center text-sm font-medium text-blue-300/80 mb-2">
                    ...
                    {IconComponent && (
                        <IconComponent className="h-5 w-5 mr-2 flex-shrink-0" aria-hidden="true" />
                    )}
                    <h4 className="truncate" title={label}>
                        {label}
                    </h4>
                </div>
                ...
                {id === 'topGenres' && Array.isArray(value) ? (
                    <div className="flex flex-wrap gap-1.5 mt-1">
                        {value.map(({ genre, count }) => (
                            <span
                                key={genre}
                                className="px-3 py-1 bg-slate-700/50 text-blue-300 text-xs font-medium rounded-full whitespace-nowrap"
                            >
                                {genre}{' '}
                    ...
                    <p
                        className={`text-2xl xl:text-3xl font-semibold ${valueClassName} break-words`}
                    >
            ...
            {description && <p className="text-xs text-slate-400/80 mt-2">{description}</p>}
        </AccentCard>
    );
};
