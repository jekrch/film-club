...
import { Film, ClubRating, MovieClubDetails } from '../types/film';
...
describe('statUtils', () => {
    ...
    describe('calculateMemberStats', () => {
        ...
        const filmsData: Film[] = [
            makeFilm({
                ...
                movieClubInfo: makeClubInfo({
                    ...
                    clubRatings: [
                        makeRating({ user: 'Alice', score: 8 }),
                        makeRating({ user: 'Bob', score: 7 }),
                        makeRating({ user: 'Charlie', score: 9 }),
                    ],
                }),
            }),
            makeFilm({
                ...
                movieClubInfo: makeClubInfo({
                    selector: 'Bob',
                    watchDate: '2023-02-01',
                    clubRatings: [
                        makeRating({ user: 'Alice', score: 6 }),
                        makeRating({ user: 'Bob', score: 8 }),
                    ],
                }),
            }),
            makeFilm({
                ...
                movieClubInfo: makeClubInfo({
                    selector: 'Alice',
                    watchDate: '2023-03-01',
                    clubRatings: [
                        makeRating({ user: 'Alice', score: 9 }),
                        makeRating({ user: 'Bob', score: 9 }),
                        makeRating({ user: 'Charlie', score: null }), // Null score
                    ],
                }),
            }),
            ...
            makeFilm({
                ...
                movieClubInfo: makeClubInfo({
                    ...
                    clubRatings: [makeRating({ user: 'Alice', score: 7 })],
                }),
            }),
            ...
            makeFilm({
                ...
                movieClubInfo: makeClubInfo({
                    ...
                    clubRatings: [
                        makeRating({ user: 'Alice', score: 5 }),
                        makeRating({ user: 'Bob', score: 6 }),
                    ],
                }),
            }),
        ];
        ...
        it('should calculate stats correctly for a member', () => {
            ...
            expect(mockCalculateClubAverage).toHaveBeenCalledWith(
                filmsData[0].movieClubInfo?.clubRatings
            );
            expect(mockCalculateClubAverage).toHaveBeenCalledWith(
                filmsData[2].movieClubInfo?.clubRatings
            );
            expect(mockCalculateClubAverage).toHaveBeenCalledWith(
                filmsData[4].movieClubInfo?.clubRatings
            );
            ...
            expect(statsAlice.avgGivenScore).toBeCloseTo(
                aliceGivenScores.reduce((a, b) => a + b, 0) / aliceGivenScores.length
            ); // (8+6+9+7+5)/5 = 7
            ...
            expect(statsAlice.avgSelectionYear).toBeCloseTo(
                aliceSelectionYears.reduce((a, b) => a + b, 0) / aliceSelectionYears.length
            );
        });
        ...
        it('should calculate stats correctly for another member (Bob)', () => {
            ...
            expect(mockCalculateClubAverage).toHaveBeenCalledWith(
                filmsData[1].movieClubInfo?.clubRatings
            );
            ...
            expect(statsBob.avgGivenScore).toBeCloseTo(
                bobGivenScores.reduce((a, b) => a + b, 0) / bobGivenScores.length
            ); // (7+8+9+6)/4 = 7.5
        ...
        it('should handle films with missing movieClubInfo or clubRatings gracefully', () => {
            const filmsWithMissingData: Film[] = [
                // `clubRatings` is required on the type but genuinely absent in data
                // the sheet has produced, so the omission is stated as a cast on the
                // literal rather than left to a missing property — which is what this
                // test is about.
                makeFilm({
                    title: 'Film X',
                    imdbID: 'tt00X',
                    year: '2024',
                    movieClubInfo: { selector: 'David' } as unknown as MovieClubDetails,
                }),
                makeFilm({ title: 'Film Y', imdbID: 'tt00Y', year: '2024' }),
            ];
            ...
            const filmsForAvgTest: Film[] = [
                makeFilm({
                    ...
                    movieClubInfo: makeClubInfo({
                        selector: 'Tester',
                        clubRatings: [
                            makeRating({ user: 'Tester', score: 8 }),
                            makeRating({ user: 'Other', score: 7 }),
                        ],
                    }),
                    // Required on the type, missing in practice — see the cast above.
                    runtime: null as unknown as string,
                }),
                makeFilm({
                    title: 'No Score Film',
                    imdbID: 'sc01',
                    ...
                    movieClubInfo: makeClubInfo({
                        selector: 'Tester',
                        clubRatings: [makeRating({ user: 'Tester', score: null })],
                    }),
                    runtime: '100 min',
                }),
                makeFilm({
                    ...
                    imdbID: 'yr01',
                    year: '',
                    movieClubInfo: { selector: 'Tester' } as unknown as MovieClubDetails,
                    runtime: '100 min',
                }),
            ];
            mockCalculateClubAverage.mockImplementation((ratings: ClubRating[] | undefined) => {
                if (
                    ratings &&
                    ratings.length > 0 &&
                    ratings[0].user === 'Tester' &&
                    ratings[0].score === 8
                )
                    return 7.5; // for No Runtime Film
                return null;
            });
