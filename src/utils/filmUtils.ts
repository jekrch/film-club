...
export const parseWatchDate = (dateString: string | null | undefined): Date | null => {
    ...
    if (month < 1 || month > 12 || day < 1 || day > 31) {
        console.warn(`Invalid month/day: ${dateString}`);
        return null;
    }
    ...
    if (year < 2000) {
        console.warn(`Year before 2000: ${dateString}`);
        return null;
    }
    try {
        ...
        if (
            ...
            console.warn(`Invalid date construction from parts: ${dateString}`);
            return null;
        }
    } catch (e) {
        console.error(`Error creating Date: ${dateString}`, e);
        return null;
    }
};

export const getAllFilmCreditsForPerson = (
    personName: string,
    allFilms: Film[]
): PersonCredit[] => {
    ...
    allFilms.forEach((f) => {
        ...
        const checkCreditField = (creditField: string | undefined, roleName: string) => {
            if (
                creditField &&
                typeof creditField === 'string' &&
                creditField
                    .toLowerCase()
                    .split(',')
                    .map((n) => n.trim())
                    .includes(trimmedPersonName)
            ) {
                addRole(roleName);
            }
        };
        ...
        if (
            Array.isArray(f.cast) &&
            f.cast.some((member) => member?.name?.trim().toLowerCase() === trimmedPersonName)
        ) {
            addRole('Actor');
        }
