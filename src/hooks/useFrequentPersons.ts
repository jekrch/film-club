...
export interface FrequentPersonDataHook extends PersonCredit {
    // Re-using PersonCredit for individual film entries
    name: string; // Person's name
    count: number; // How many films they are credited in
    filmography: PersonCredit[];
}

export interface CreditsModalInfoHook {
...
export const useFrequentPersons = (films: Film[], topN: number = 6): UseFrequentPersonsReturn => {
    ...
    useEffect(() => {
        ...
        const personCounts: Record<
            string,
            { count: number; filmEntries: PersonCredit[]; displayName: string }
        > = {};
