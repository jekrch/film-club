...
/**
 * Coordinates zoom/pan and slide gestures, routing pointer and touch events to
 * the appropriate behavior based on current zoom state. Ported from
 * comic-snaps' PanelViewer.
 *
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (e.touches.length === 2) {
        ...
        if (transformRef.current.scale > 1) {
          p.lastTouchPos = { x: e.touches[0].clientX, y: e.touches[0].clientY };
        } else {
