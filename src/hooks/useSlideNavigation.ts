 ...
 * commit/snap-back, and DOM resets on navigation. Ported from comic-snaps'
 * PanelViewer, adapted to a plain array of image URLs.
 */
export function useSlideNavigation(
  ...
  const commitSlide = useCallback(
    (direction: 'prev' | 'next') => {
      ...
      requestAnimationFrame(() => {
        ...
        const cleanup = () => {
          ...
          preload.src = images[newIndex];

          const doNavigate = () => onNavigate(newIndex);

          // Timeout so a stalled decode can't block navigation forever.
    ...
    [applySlideOffset, currentIndex, images, onNavigate],
  );
