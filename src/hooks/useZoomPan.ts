...
/**
 * Manages zoom/pan state for an image viewer. Ported from comic-snaps'
 * PanelViewer.
 *
 * Applies scale + translate transforms to the **wrapper** element rather than
 * the image itself. This avoids an iOS Safari compositing bug where CSS scale()
 * on an element clips its painted output to the element's original layout
 * bounds. When zoomed, the wrapper is positioned absolute inset-0 (full
 * viewport), so its layout bounds already match the viewport and scaling it
 * won't clip. The image stays at its natural constrained size, centered via
 * flexbox.
 */
export function useZoomPan(
  ...
  // Reset zoom when the visible image changes.
  useLayoutEffect(() => {
    const wrapper = imgWrapperRef.current;
    if (wrapper) {
