...
Data for the website is managed through JSON files, a Google Sheet, and the OMDB API:
-   `src/assets/club.json` - Club member information.
-   `src/assets/films.json` - Contains detailed film data (plot, cast, poster, etc. fetched from OMDB for new films) and club-specific information like member reviews, watch dates, and selectors (primarily synced from the Google Sheet).
...
Film data management is now largely automated through our Google Sheet and a backend sync process that uses the OMDB API for new film details.
...
5.  You can (and should\!) also pre-fill other information in the sheet for the new film:
      ...
      * Your personal rating and blurb in your respective columns (e.g., `andy_rating`, `jacob_blurb`).
        This information will be added to the film's entry in `films.json` alongside the OMDB data.
...
### Sync and Deploy (Google Sheet 🔄 OMDB 🔄 JSON 🚀 Website)
...
2.  **Sheet Processing:**
      ...
      * **Existing Films:** For IMDb IDs already present in `films.json`, it updates club-specific information (member ratings, blurbs, `watch_date`, `selected_by`, `trophy_notes`) based on any changes found in the sheet.
...
The site is available at:
  ...
  - GitHub Pages URL: [https://jekrch.github.io/film-club/](https://www.google.com/search?q=https://jekrch.github.io/film-club/)
...
3.  The **most important** piece of info: type the film's **IMDb ID** (e.g., `tt0036342`) into the `imdb_id` column.
      * *Our system will use this ID to automatically look up all the core movie details (title, director, actors, full plot, poster image, etc.) from OMDB\!*
...
5.  That's it\! The new film, enriched with OMDB data and your initial input, will appear on the website after the next automated sync.
